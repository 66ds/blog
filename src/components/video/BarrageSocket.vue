<template>
  ...
  import protobuf from 'utils/proto.js'
  ...
  export default {
    ...
    methods: {
      initWebSocket() {
        const wsUri = constants.config.wsUri;
        // console.log("this.websocket.state ", this.websocket.state)
        // if (!this.websocket.state === "Open") {
        this.websocket = new WebSocket(wsUri);
        this.websocket.binaryType = constants.config.wsBinaryType;
      ...
      heartBeatSend: function () {
        const data = {
          userId: constants.config.userId,
          videoId: constants.config.videoId,
        }
