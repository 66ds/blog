...
export default {
  data: function () {
    return {
        ...
        username: [
          {
            ...
            validator: (rule, value, callback) => {
              if (!value) {
                callback(new Error(Partten.phoneNotEmpty));
              } else if (!validPhone(value)) {
                callback(new Error(Partten.phoneError));
              } else {
                callback();
              }
            },
      ...
      flag: false,
      imgcode: "", //默认验证码图片
      imgsrc: Partten.codeUrl + Math.round(Math.random() * 100), //滑动校验图片(这里使用的是外网链接,取随机100-200的图片)
      dialogVisible: false,
  ...
  methods: {
    ...
    //校验码不通过
    verifyFail() {
      this.$message.error(Partten.codeVerifyError);
      //重新刷新校验码
      this.refreshVerify();
      //重新获取验证码
      this.getCode();
    ...
    refreshVerify() {
      ...
      //重新刷新
      this.imgsrc = Partten.codeUrl + Math.round(Math.random() * 100);
    },
    getCode() {
    ...
    closeModelToLogin() {
      userLoginApi(
        ...
        .catch((e) => {
          ...
          this.$message.error(data || Partten.loginError);
          //判断滑块存存在,如果存在重置滑块
